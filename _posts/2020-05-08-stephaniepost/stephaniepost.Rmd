...
# with such a large data set, I added the searchable function into your reacatable table to help the user more quickly identify specific values 
...
map_dbl(d2$data, ~sd(.x$fatalities))
...
#I think a function like the one developed for lab 4 should solve this problem, but I'm not confident on how to get it to run

means_cw <- function(x) {
  if(!is.numeric(pull(x)))
    stop("Supplied column is not numeric")
  x %>% 
    summarize(mean(x))
}

means_cw(d2$fatalities)
